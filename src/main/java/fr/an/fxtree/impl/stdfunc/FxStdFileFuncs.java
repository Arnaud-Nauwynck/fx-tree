...
public final class FxStdFileFuncs {
    ...
    public static class FxImportJsonFileFunc extends FxNodeFunc {
        ...
        public void eval(FxChildWriter dest, FxEvalContext ctx, FxNode src) {
            ...
            FxSourceLoc source = new FxSourceLoc("", file.getAbsolutePath());
            FxJsonUtils.readTree(dest, file, source);
        }
    }
    ...
    public static class FxImportYamlFileFunc extends FxNodeFunc {
        ...
        public void eval(FxChildWriter dest, FxEvalContext ctx, FxNode src) {
            ...
            FxSourceLoc source = new FxSourceLoc("", file.getAbsolutePath());
            FxYamlUtils.readTree(dest, file, source);
        }
    }
    ...
    public static class FxImportFileFunc extends FxNodeFunc {
        ...
        public void eval(FxChildWriter dest, FxEvalContext ctx, FxNode src) {
            ...
            FxSourceLoc source = new FxSourceLoc("", file.getAbsolutePath());
            FxFileUtils.readTree(dest, file, source);
        }
    }
    ...
    public static class FxScanDirImportFilesFunc extends FxNodeFunc {
        ...
        public void eval(FxChildWriter dest, FxEvalContext ctx, FxNode src) {
            ...
            FxSourceLoc loc = FxSourceLoc.newFrom(NAME, src.getSourceLoc(),
                    new FxSourceLoc("", file.getAbsolutePath()));
            ...
            if (file.isDirectory()) {
                if (! recursive) {
                    for(File f : file.listFiles()) {
                        ...
                        addImportFile(childWriter, file, continueOnError, loc);
                    }
                } else {
                    recursiveScanDir(file, childWriter, pattern, continueOnError, loc);
                }
            } else {
                FxJsonUtils.readTree(dest, file, loc);
            }
        }
        
        protected void recursiveScanDir(File currDir, 
                FxChildWriter childWriter, Pattern pattern, boolean continueOnError,
                FxSourceLoc loc) {
            for(File f : currDir.listFiles()) {
                if (f.isDirectory()) {
                    recursiveScanDir(f, childWriter, pattern, continueOnError, loc);
                } else {
                    if (! pattern.matcher(f.getName()).matches()) {
                        continue;
                    }
                    addImportFile(childWriter, f, continueOnError, loc);
                }
            }
        }
        
        protected void addImportFile(FxChildWriter childWriter, 
                File file, boolean continueOnError, 
                FxSourceLoc loc) {
            try {
                FxJsonUtils.readTree(childWriter, file, loc);
            } catch(Exception ex) {
                if (continueOnError) {
